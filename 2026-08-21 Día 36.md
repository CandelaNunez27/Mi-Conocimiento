...
1. Docker:
	...
	Volvemos a la carpeta clase 2 DockerFile `docker build -t servidor-ssh-demo .`  `docker image ls`
	
	`docker run -d --name contenedor_seguro -p 2222:22 servidor-ssh-demo`
	
	`docker ps`
	
	`ssh -i ~/.ssh/id_demo_ssh -p 2222 adminlab@localhost` para conectarnos le damos 
