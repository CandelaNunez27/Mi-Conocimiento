
### Preparación

1. Crear cuenta en https://tailscale.com/

![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001118.png)

2. Usa el siguiente enlace para agregar el host bastión a tu red de Tailscale: **[https://login.tailscale.com/admin/invite/SFneanC8udULcaAz7ymb11](https://login.tailscale.com/admin/invite/SFneanC8udULcaAz7ymb11)** 
   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001236.png)
   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001302.png)
   
   3. Descargar desde **[https://tailscale.com/download](https://tailscale.com/download)** 
	   `curl -fsSL https://tailscale.com/install.sh | sh` 
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001445.png)
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001512.png)
	   
	   nos loging en la consola
	   `sudo tailscale up
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001841.png)
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001854.png)
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001905.png)
	   ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916001935.png)

2. Nos conectamos al bastion por ssh con su nombre de dominio completo:
	`ssh scct-2613@bastion.tail263e10.ts.net` 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260916002516.png)
	







--- 
**

SCC@CARLA

Acceso a la Infraestructura

Introducción


A cada equipo se le ha otorgado acceso a tres nodos del clúster Helvetios. Cada nodo está equipado con dos procesadores Intel Xeon Gold 6140, 192 GB de RAM y un SSD de 800 GB. Este documento describe cómo acceder a la infraestructura y cómo instalar un sistema operativo en sus nodos asignados.

# hola

# Parte 0: Definiciones

Antes de comenzar, aquí hay algunas definiciones importantes.

  

Diagrama de infraestructura

  ![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260915235955.png)

Los equipos accederán a un host bastión compartido a través de una VPN usando Tailscale. Desde el host bastión, podrán acceder a sus nodos asignados y a sus BMCs correspondientes.

# hola

Esquema: equipos -> Internet (tailscale) -> nodo bastión -> BMCs y Nodos Helvetios



TID
# 
A cada equipo se le ha asignado un ID de equipo. A lo largo de este documento, TID se refiere a los últimos dos dígitos del ID del equipo. Por ejemplo, si el ID del equipo es scct2612, entonces TID = 12.

# Parte 1: Acceso al Host Bastión

El primer paso es conectarse al host bastión. Esta es una máquina compartida que es accesible desde Internet a través de Tailscale.

  

1. Crea una cuenta de Tailscale: [Enlace a Tailscale](https://tailscale.com/)
    
2. Usa el siguiente enlace para agregar el host bastión a tu red de Tailscale: File
    
3. Instala Tailscale en tu computadora: [Descarga Tailscale](https://tailscale.com/download)
    
4. Inicia sesión. En Linux, por ejemplo:  
    sudo tailscale up
    
5. Una vez que Tailscale esté conectado, ingresa por SSH al host bastión:  
    ssh scct-XXXX@bastion.tail263e10.ts.net
    

  

Cada equipo tiene una única cuenta en el host bastión. Las claves públicas SSH del equipo ya han sido agregadas a la cuenta.

# 
# Parte 2: Acceso a los BMCs

Cada equipo tiene acceso al Controlador de Gestión de Placa Base (BMC) de sus nodos asignados. Los nodos usan HPE iLO para la administración remota, lo que incluye control de energía, acceso a la consola remota y medios virtuales.

  

La red de BMC de cada equipo sigue el esquema de direcciones 10.1.TID.NODE, donde NODE es 1, 2, o 3. Las credenciales del BMC han sido proporcionadas a cada equipo en privado. Puedes acceder al BMC ya sea a través de la interfaz web o usando el comando ilorest.

## Parte 2a: Interfaz Web de iLO

La red del BMC no es accesible directamente desde Internet. Para acceder a la interfaz web, crea un túnel SSH a través del host bastión. Por ejemplo:

  

ssh -L 8000:10.1.TID.NODE:443 scct-XXXX@bastion.tail263e10.ts.netLuego abre la siguiente URL en tu navegador: localhost en el puerto 8000.

  

Ingresa las credenciales del BMC proporcionadas a tu equipo.

  

Nota: Es normal que la interfaz web de iLO muestre una advertencia de certificado HTTPS. En el navegador, selecciona Avanzado -> Proceder (Riesgoso) para acceder a la interfaz de iLO.

## Parte 2b: ilorest

Si prefieres usar la línea de comandos, ilorest ya está instalado en el host bastión. Inicia sesión en el BMC con:

  

$ ilorest login 10.1.TID.NODE -u <span type="placeholder" placeholder-type="person"></span> -p <span type="placeholder" placeholder-type="person"></span>Reemplaza la dirección IP y las credenciales con las proporcionadas a tu equipo.

# Parte 3: Arrancar una ISO

Para instalar un sistema operativo, usaremos la funcionalidad de Medios Virtuales proporcionada por iLO. Primero, descarga la imagen ISO que quieres instalar. Por ejemplo:

  

$ wget <span type="placeholder" placeholder-type="file"></span>El BMC necesita poder descargar la ISO a través de la red. Para hacer esto, ejecuta un servidor HTTP ligero en el host bastión. Por ejemplo:

  

$ python3 -m RangeHTTPServer <span type="placeholder" placeholder-type="person"></span> --bind <span type="placeholder" placeholder-type="place"></span><local_bastion_ip> es la dirección IP del host bastión que es alcanzable desde la red del BMC. Esta dirección ha sido proporcionada a cada equipo. Recomendamos usar el puerto 8000+TID para port. Esto evita conflictos de puertos entre los equipos. Por ejemplo, el equipo scct-2612 debería usar el puerto 8012.

  

La ISO entonces debería estar disponible para el BMC en: IP del bastión local : puerto / Rocky-10.2-x86_64-minimal.iso

## Parte 3a: Arrancar una ISO Usando la Interfaz Web

En la interfaz web de iLO, ve a:  
Remote Console & Media -> Virtual Media -> Connect CD/DVD-ROM

  

Ingresa la URL de la imagen ISO. Una vez que el medio virtual esté conectado, enciende o reinicia el nodo y arranca desde la unidad de CD/DVD virtual.

## Parte 3b: Arrancar una ISO Usando ilorest

También puedes configurar el medio virtual desde la línea de comandos:

  

$ ilorest virtualmedia 2 "<span type="placeholder" placeholder-type="file"></span>" --bootnextresetLuego reinicia el nodo:

  

$ ilorest rebootEl nodo debería arrancar desde la ISO montada en su siguiente reinicio.

# Parte 4: Instalar un Sistema Operativo

Instala el sistema operativo de forma normal usando la consola remota de iLO. Durante la instalación, asegúrate de:

  

- Configurar la red de acuerdo con las instrucciones en la Parte 5.
    
- Crear un usuario no root con acceso a sudo con una contraseña fuerte.
    
- Habilitar el acceso SSH.
    
- Deshabilitar el inicio de sesión directo por SSH para el usuario root.
    

# Parte 5: Configuración de Red

## Ethernet

A cada equipo se le ha asignado una red /24 para sus nodos. La red de tu equipo es: 10.2.TID.0/24

  

La configuración de la red debería usar:

  

- Red: 10.2.TID.0/24
    
- Gateway: 10.2.TID.254
    
- DNS: 10.2.TID.254
    

  

Elige una dirección IP única dentro de tu red asignada para cada nodo. No uses la dirección del gateway (10.2.TID.254). Después de configurar la red, verifica la conectividad desde el host bastión:

  

ping 10.2.TID.NODE
ssh username@10.2.TID.NODE

## Infiniband

El Gestor de Subred de InfiniBand es administrado por el equipo de infraestructura de SCC@CARLA. Los equipos no necesitan configurar ni ejecutar OpenSM por sí mismos. Asegúrate de que tus interfaces y conductores de InfiniBand estén instalados correctamente y verifica que las interfaces estén operativas antes de la competencia.

# Parte 6: Configuración de la BIOS

Antes de la final, la configuración de la BIOS de todos los nodos se restablecerá a la línea base de la competencia. Si tu equipo ha hecho algún cambio en la configuración de la BIOS, asegúrate de respaldar tus configuraciones de BIOS antes de la fecha límite de acceso al bastión. Puedes usar ilorest para guardar y restaurar la configuración de la BIOS.

  

Respaldo

  

Inicia sesión en el BMC como se explica en la sección Parte 2b. Guarda la configuración actual de la BIOS:

  

ilorest save --selector Bios. -f bios.jsonEsto creará un archivo de configuración que contiene los ajustes actuales.

  

Restaurar

  

Al inicio de la final, inicia sesión en el BMC y carga la configuración guardada:

  

ilorest load -f bios.jsonPuede requerirse un reinicio para que los cambios surtan efecto.

# Parte 7: Reglas

Romper cualquiera de las reglas establecidas en este documento puede resultar en la descalificación.

## Parte 7a

Los Asesores de Equipo no tienen permitido acceder a la infraestructura del clúster. Los miembros del equipo deben configurar la infraestructura por su cuenta.

## Parte 7b

El host bastión y la infraestructura del clúster son recursos compartidos. Las siguientes acciones están estrictamente prohibidas:

  

- Ejecutar software de minería de criptomonedas en el host bastión.
    
- Ejecutar cargas de trabajo o benchmarks en el host bastión.
    
- Escanear, atacar, o interferir de cualquier manera con las direcciones IP o infraestructura de otros equipos.
    
- Intentar obtener acceso root en infraestructura que no pertenece a tu equipo.
    
- Interrumpir intencionalmente servicios usados por otros equipos.
    
- Usar la infraestructura para propósitos no relacionados con la competencia.
    

  

Tienes permitido ejecutar herramientas de configuración y automatización desde el host bastión (por ejemplo, usar playbooks de Ansible). Regla general: Eres libre de experimentar con tus propios nodos, pero no interfieras con el host bastión, otros equipos, o infraestructura que no te haya sido asignada.

# Parte 8: Cronograma de la competencia

La final se llevará a cabo el martes Date y el miércoles Date. El host bastión se volverá inaccesible el viernes Date a las 23:59, y permanecerá inaccesible hasta la final.

  

Asegúrate de que todos los sistemas operativos, software, configuración, redes y automatización estén completamente instalados y probados antes de la fecha límite.

















---

### Preparación con tailscale

1. Paquetes necesarios
	`sudo pacman -Syu qemu-full virt-manager virt-viewer dnsmasq openbsd-netcat swtpm iptables-nft`
	![](data:image/png;base64,)

2. Activar los servicios necesarios:
	Una vez que termine la instalación, activar el motor de las máquinas virtuales
	`sudo systemctl enable --now libvirtd`
	![](../../../../04%20-%20Otros/Imagenes/Pasted%20image%2020260902023808.png)

3. Permisos de usuario
	Agrega tu usuario al grupo `libvirt` para que no tengas que usar la contraseña de root cada vez que abras el programa 
	
	`sudo usermod -aG libvirt,libvirt-host $USER`
	![](../../../../04%20-%20Otros/Imagenes/Pasted%20image%2020260902024859.png)
	
	Luego cerrar sesión o reiniciar PC para que se apliquen los cambios.
	

3. ISO y Gestor de Máquinas Virtuales
	Antes de seguir descargamos Rocky Linux 9 (Minimal ISO) desde https://rockylinux.org/download (activar vpn en ee.uu para una descarga rápida)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260903230535.png)

4. Gestor de Máquinas Virtuales
	Luego del reinicio, en las Apps nos saldra una llamada Gestor de Máquinas Virtuales (virt-manager) la cual abriremos
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904013234.png)
	
	
	**nodo-master (Servicios, Ansible y Compilación Base)**: Le damos a crear nueva maquina virtual > Medio de instalación local > siguiente > seleccionamos la iso que descargamos > siguiente > memoria 3815 MiB y 2 nucleos de CPU > siguiente> dejar marcaado "habilitar almacenamiento para la mv" y asignar 20 GiB > siguente  > marcamos personalizar configuración antes de instalar > en red le colocamos  
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904013405.png) 
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904013706.png) 
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904014548.png)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904014909.png)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904015622.png)
	
	
	Como nos salia la nat inactiva la activamos con este comando `sudo virsh net-start default` y `sudo virsh net-autostart default` para que arranque cada vez que prenda la computadora.
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904015422.png)
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904015552.png)
	
	Para que deje de salir inactiva una vez ya acticada cancele todo y volve a realizar los pasos.
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904015811.png) 
	Luego nos sale un menu de configuración avanzada. Donde nos nos vamos a CPU > y nos aseguramos que este marcada la opción Copiar configuración de la CPU del anfitrión (host-passthrough) > iniciar la instalación
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904020310.png)
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904020339.png)
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904021026.png)
	
	Creamos las otras maquinas virtuales con el mismo hardware y el mismo proceso. Por ende nos quedara las tres maquinas virtuales listas para usar (nodo-master, nodo-calculo1, node-calculo2) 

5. Instalación de Rocky linux minimal 9.8
	Arrancamos la maquina node-master y nos saldra con interfaz gráfica la instalación.
	Seleccionamos idioma Español (Argentina) > seleccionamos teclado > creación de usuario > hpc-user , LocalHPC-1234, marcamos Hacer de este usuario un administrador > Luego en contraseña de root le cole que la misma > selecionamos el disco de 20 y que quede en las particionado automatico > en red le colocamos el nombre del equipo nodo-master y la red > configurar...> ajustes de IPv4 > manual, añadir , direccion: 192.168.122.10, mascara: 255.255.255.0 , puerta de enlace: 192.168.122.1, DNS: 8.8.8.8 > guardar y conectar. Volvemos al menu del resumen de instalación y le damos a Comenzar la instalación > Reiniciar el sistema.
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904022215.png)
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904022503.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904022318.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904022939.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904023418.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904023649.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904024851.png) 
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904024449.png)  
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904025019.png)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904025238.png)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904030238.png)
	
	![](../../04%20-%20Otros/Imagenes/Pasted%20image%2020260904030347.png)

